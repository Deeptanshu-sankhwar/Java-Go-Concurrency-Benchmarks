...
-   **Metrics Evaluated**:
    ...
    3.  **CPU Utilization**: Tracks CPU usage during task execution using:
        -   Java: `Runtime.getRuntime()` for memory and `JConsole` for CPU profiling.
        -   Go: `runtime.ReadMemStats` for memory and `gopsutil` for CPU profiling.
    4.  **Concurrency Overhead**: Measures the time difference between task completion and total concurrent overhead.
-   **Key Results**:

    -   Go's goroutines demonstrate significantly lower concurrency overhead compared to Java's thread-based model.
    -   Java, with proper JVM tuning and preloading optimizations, narrows the performance gap but remains slightly slower in execution time for large datasets.
    -   Both models perform similarly in terms of memory usage, but Go's lightweight goroutines scale better under higher concurrency.
